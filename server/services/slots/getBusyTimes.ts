...
export async function getBusyTimes(params: {
  ...
  const endTimeDate =
    rescheduleUid && duration
      ? dayjs(endTime).add(duration, "minute").toDate()
      : new Date(endTime);
  ...
  const busyTimes = bookings.reduce(
    (
      ...
      { id, startTime, endTime, eventType, title, ...rest }
    ) => {
      if (rest._count?.seatsReferences) {
        const bookedAt =
          dayjs(startTime).utc().format() +
          "<>" +
          dayjs(endTime).utc().format();
      ...
      aggregate.push({
        start: dayjs(startTime)
          .subtract(
            (eventType?.beforeEventBuffer || 0) + (afterEventBuffer || 0),
            "minute"
          )
          .toDate(),
        end: dayjs(endTime)
          .add(
            (eventType?.afterEventBuffer || 0) + (beforeEventBuffer || 0),
            "minute"
          )
          .toDate(),
        title,
  ...
  performance.measure(
    `prisma booking get took $1'`,
    "prismaBookingGetStart",
    "prismaBookingGetEnd"
  );
  if (credentials?.length > 0) {
    ...
    if (rescheduleUid) {
      const originalRescheduleBooking = bookings.find(
...
export async function getBusyTimesForLimitChecks(params: {
  ...
  const busyTimes = bookings.map(
    ({ id, startTime, endTime, eventType, title }) => ({
  ...
  logger.silly(
    `Fetch limit checks bookings for eventId: ${eventTypeId} ${JSON.stringify(
      busyTimes
    )}`
  );
