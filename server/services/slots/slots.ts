...
// import type {
//   WorkingHours,
//   TimeRange as DateOverride,
// } from "@calcom/types/schedule";

import { getWorkingHours } from "../calendar/availablity";
import { getTimeZone } from "../date-fns";
import { DateRange } from "../calendar/date-ranges";
import { TimeRange as DateOverride, WorkingHours } from "../../types/schedule";
...
function buildSlots({
  ...
  const dayStart = startOfInviteeDay.isSame(startDate, "day")
    ? Math.ceil((startDate.hour() * 60 + startDate.minute()) / frequency) *
      frequency
    : 0;
  ...
  const boundaries = computedLocalAvailability
    .map((item) => [
      item.startTime < dayStart ? dayStart : item.startTime,
      item.endTime,
    ])
    .sort((a, b) => a[0] - b[0]);
    ...
    for (
      ...
      computedLocalAvailability.forEach((item) => {
        ...
        if (
          ...
          slotStart > item.endTime + 1 - eventLength
        ) {
          return;
        }
        slotsTimeFrameAvailable[slotStart.toString()] = {
          userIds: (slotsTimeFrameAvailable[slotStart]?.userIds || []).concat(
            item.userIds || []
          ),
          startTime: slotStart,
  ...
  const organizerDSTDiff =
    dayjs().tz(organizerTimeZone).utcOffset() -
    startOfInviteeDay.tz(organizerTimeZone).utcOffset();
  const inviteeDSTDiff =
    dayjs().tz(inviteeTimeZone).utcOffset() -
    startOfInviteeDay.tz(inviteeTimeZone).utcOffset();
...
function fromIndex<T>(
  cb: (val: T, i: number, a: T[]) => boolean,
  index: number
) {
...
const getSlots = ({
  ...
  const activeOverrides = dateOverrides.filter((override) => {
    return dayjs
      .utc(override.start)
      .isBetween(startOfInviteeDay, startOfInviteeDay.endOf("day"), null, "[)");
  });
  ...
  if (activeOverrides.length) {
    const overrides = activeOverrides.flatMap((override) => ({
      ...
      startTime:
        override.start.getUTCHours() * 60 + override.start.getUTCMinutes(),
    ...
    overrides.forEach((override) => {
        ...
        (i = computedLocalAvailability.findIndex(
          fromIndex(
            (a) =>
              !a.userIds?.length ||
              (!!override.userIds[0] &&
                a.userIds?.includes(override.userIds[0])),
            i + 1
          )
        )) != -1
      ) {
      ...
      indexes
        .reverse()
        .forEach((idx) => computedLocalAvailability.splice(idx, 1));
    });
