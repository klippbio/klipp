...
export const getUserAvailability =
  async function getUsersWorkingHoursLifeTheUniverseAndEverythingElse(
    ...
    if (!user)
      throw new HttpError({ statusCode: 404, message: "No user found" });
    ...
    const schedule =
      !eventType?.metadata?.config?.useHostSchedulesForTeamEvent &&
      eventType?.schedule
        ? eventType.schedule
        : userSchedule;
    ...
    logger.debug(
      `getWorkingHours took ${
        endGetWorkingHours - startGetWorkingHours
      }ms for userId ${userId}`
    );
...
class LimitManager {
  ...
  isAlreadyBusy(start: Dayjs, unit: IntervalLimitUnit) {
    ...
    if (
      ...
      ((this.busyMap.has(LimitManager.createKey(start, "month")) &&
        this.busyMap.has(
          LimitManager.createKey(start.endOf("week"), "month")
        )) ||
...
const getBusyTimesFromLimits = async (
  ...
  for (const key of ["PER_MONTH", "PER_WEEK", "PER_DAY"] as Exclude<
    keyof IntervalLimit,
    "PER_YEAR"
  >[]) {
  ...
  if (bookingLimits) {
    ...
    performance.measure(
      `checking booking limits took $1'`,
      "bookingLimitsStart",
      "bookingLimitsEnd"
    );
  }
  ...
  if (durationLimits) {
    ...
    performance.measure(
      `checking duration limits took $1'`,
      "durationLimitsStart",
      "durationLimitsEnd"
    );
  }
    ...
    for (const periodStart of periodStartDates) {
      ...
      if (unit === "year") {
        try {
          ...
          if (
            periodStartDates.every((start) =>
              limitManager.isAlreadyBusy(start, unit)
            )
          ) {
            return;
          }
  ...
  for (const key of descendingLimitKeys) {
    ...
    for (const periodStart of periodStartDates) {
      ...
      for (const booking of bookings) {
        ...
        totalDuration += dayjs(booking.end).diff(
          dayjs(booking.start),
          "minute"
        );
