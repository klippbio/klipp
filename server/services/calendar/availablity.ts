...
import type { ConfigType } from "../../utils/dayjs.index";
import dayjs from "../../utils/dayjs.index";
import type { Schedule, TimeRange, WorkingHours } from "../../types/schedule";
import type {
  ...
  const weekSpan = (availability: Availability) => {
    ...
    return days
      .map((dayRange) =>
        dayRange
          .map((day) =>
            new Intl.DateTimeFormat("short").format(new Date(1970, 0, day + 4))
          )
          .join(" - ")
      )
      .join(", ");
  ...
  const timeSpan = (availability: Availability) => {
    return (
      new Intl.DateTimeFormat(undefined, {
        ...
        minute: "numeric",
        hour12,
      }).format(new Date(availability.startTime.toISOString().slice(0, -1))) +
      " - " +
      new Intl.DateTimeFormat(undefined, {
        // Pass 'undefined' as the locale
        hour: "numeric",
