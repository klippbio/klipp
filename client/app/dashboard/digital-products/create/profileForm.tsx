...
export function ProfileForm({
  ...
  const { data: stripeDetails, isLoading: stripeDetailsLoading } = useQuery(
  ...
  const checkStripeValidity = async (
    data: z.infer<typeof digitalDownloadsSchema>
  ) => {
    if (
      (stripeDetails?.onboardingComplete === undefined ||
        stripeDetails?.onboardingComplete === false) &&
      ...
      toast({
        ...
        duration: 2000,
        description:
          "Please complete your stripe onboarding to list a paid product.",
      });
      return false;
    }
    return true;
  ...
  const mutation = useMutation({
    mutationFn: async (data: z.infer<typeof digitalDownloadsSchema>) => {
      ...
      const isValid = await checkStripeValidity(combinedData);
      if (!isValid) {
        // Explicitly throw an error when isValid is false
        throw new Error(
          "Please complete your stripe onboarding to list a paid public product."
        );
      }
      // Proceed with the API call only if isValid is true
    ...
    onError: () => {
      // Make sure to display an error toast that reflects the error caught
      toast({
        title: "Error",
        variant: "destructive",
        duration: 2000,
                        ...
                        render={({ field }) => (
                          <FormItem className="flex flex-row items-center mt-10 justify-between rounded-lg border p-3 shadow-sm mb-6">
                            <div className="space-y-0.5">
                              ...
                              <FormDescription>
                                Add a link to your product instead of uploading
                                a file.
                              </FormDescription>
                            </div>
                            <FormControl>
