...
export const getTimeListForSchedule = () => {
  ...
  for (let hours = 0; hours < 24; hours++) {
    for (let minutes = 0; minutes < 60; minutes += 15) {
      const label = format12Hour(hours, minutes);
      const value = new Date(
        new Date().setUTCHours(hours, minutes, 0, 0)
      ).toISOString();
      timeList.push({ value, label });
    }
  }
...
// Function to convert incoming dateOverrides to a simple array of dates
export const parseDateOverrides = (
  dateOverrides: Array<{ ranges: Array<{ start: string; end: string }> }>
) => {
  return dateOverrides.flatMap((d) => d.ranges.map((r) => new Date(r.start)));
};

export const formatDateOverridesForUpdate = (
  dates: Date[] | undefined
): Array<{ start: string; end: string }> => {
  return (
    dates?.map((date) => ({
      start: date.toISOString(),
      end: new Date(date.getTime() + 86400000).toISOString(),
    })) || []
  );
};
